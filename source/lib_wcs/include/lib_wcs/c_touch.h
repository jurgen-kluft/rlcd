...
#define TP_CT_MAX_TOUCH 10
        ...
        {
            touch_point_t points[TP_CT_MAX_TOUCH];  // Capacitive touch supports up to 10 coordinate groups
        ...
        static inline bool          tp_is_portrait(const touch_panel_t& tp) { return (tp.touchtype & 0x01) == 0; }
        static inline bool          tp_is_landscape(const touch_panel_t& tp) { return (tp.touchtype & 0x01) != 0; }
        static inline bool          tp_is_point_active(const touch_panel_t& tp) { return (tp.touchstatus & 0x4000) != 0; }
        static inline u8            tp_get_touch_point_num(const touch_panel_t& tp) { return tp.touchstatus & 0x3FF; }
        static inline bool          tp_is_capacitive(const touch_panel_t& tp) { return (tp.touchtype & 0x80) != 0; }
        static inline bool          tp_is_valid_touch_point(const touch_panel_t& tp, u8 index) { return (index < TP_CT_MAX_TOUCH) && (tp.touchstatus & (1 << index)) != 0; }
        static inline touch_point_t tp_get_touch_point(const touch_panel_t& tp, u8 index) { return tp_is_valid_touch_point(tp, index) ? tp.points[index] : touch_point_t{0xFFFF, 0xFFFF}; }
