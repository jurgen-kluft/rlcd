...
Arduino_TFT::Arduino_TFT(Arduino_DataBus *bus, int8_t rst, uint8_t r, bool ips, int16_t w, int16_t h, uint8_t col_offset1, uint8_t row_offset1, uint8_t col_offset2, uint8_t row_offset2)
    : Arduino_GFX(w, h)
    , _bus(bus)
    , _rst(rst)
    , _ips(ips)
...
void Arduino_TFT::startWrite() { _bus->beginWrite(); }
...
void Arduino_TFT::writeRepeat(uint16_t color, uint32_t len) { _bus->writeRepeat(color, len); }
...
void Arduino_TFT::writeFillRectPreclipped(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
#ifdef ESP8266
    yield();
#endif
...
void Arduino_TFT::endWrite() { _bus->endWrite(); }
...
void Arduino_TFT::writeColor(uint16_t color) { _bus->write16(color); }

// TFT optimization code, too big for ATMEL family
#if !defined(LITTLE_FOOT_PRINT)

void Arduino_TFT::writeBytes(uint8_t *data, uint32_t len) { _bus->writeBytes(data, len); }

void Arduino_TFT::writePixels(uint16_t *data, uint32_t len) { _bus->writePixels(data, len); }
...
void Arduino_TFT::writeIndexedPixels(uint8_t *bitmap, uint16_t *color_index, uint32_t len) { _bus->writeIndexedPixels(bitmap, color_index, len); }

void Arduino_TFT::writeIndexedPixelsDouble(uint8_t *bitmap, uint16_t *color_index, uint32_t len) { _bus->writeIndexedPixelsDouble(bitmap, color_index, len); }
...
void Arduino_TFT::drawBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h, uint16_t color, uint16_t bg)
{
...
void Arduino_TFT::drawBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h, uint16_t color, uint16_t bg)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
...
void Arduino_TFT::drawGrayscaleBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
...
void Arduino_TFT::drawGrayscaleBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
...
void Arduino_TFT::drawIndexedBitmap(int16_t x, int16_t y, uint8_t *bitmap, uint16_t *color_index, int16_t w, int16_t h, int16_t x_skip)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
...
void Arduino_TFT::draw16bitRGBBitmapWithMask(int16_t x, int16_t y, uint16_t *bitmap, uint8_t *mask, int16_t w, int16_t h)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
...
void Arduino_TFT::draw16bitRGBBitmap(int16_t x, int16_t y, const uint16_t bitmap[], int16_t w, int16_t h)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
...
void Arduino_TFT::draw16bitRGBBitmap(int16_t x, int16_t y, uint16_t *bitmap, int16_t w, int16_t h)
{
    if (((y + h - 1) < 0) ||  // Outside top
        (y > _max_y)          // Outside bottom
    )
    {
        return;
    ...
    {
        if ((x > _roundMaxX[y + h - 1]) &&         // top left
...
void Arduino_TFT::draw16bitBeRGBBitmap(int16_t x, int16_t y, uint16_t *bitmap, int16_t w, int16_t h)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
...
void Arduino_TFT::draw16bitBeRGBBitmapR1(int16_t x, int16_t y, uint16_t *bitmap, int16_t w, int16_t h)
{
    if (((x + h - 1) < 0) ||  // Outside left
        ((y + w - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
        (y > _max_y)          // Outside bottom
    )
...
void Arduino_TFT::draw24bitRGBBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
...
void Arduino_TFT::draw24bitRGBBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
