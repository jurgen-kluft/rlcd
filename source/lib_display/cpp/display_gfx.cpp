...
#include "lib_display/font/glcdfont.h"
#include "float.h"
#ifdef __AVR__
...
*/
/**************************************************************************/
void Arduino_GFX::fillScreen(uint16_t color) { fillRect(0, 0, _width, _height, color); }
...
*/
/**************************************************************************/
void Arduino_GFX::drawBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h, uint16_t color)
{
...
*/
/**************************************************************************/
void Arduino_GFX::drawBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h, uint16_t color, uint16_t bg)
{
    int16_t byteWidth = (w + 7) / 8;  // Bitmap scanline pad = whole byte
    uint8_t byte      = 0;
...
*/
/**************************************************************************/
void Arduino_GFX::drawBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h, uint16_t color)
{
    int16_t byteWidth = (w + 7) / 8;  // Bitmap scanline pad = whole byte
    uint8_t byte      = 0;
...
*/
/**************************************************************************/
void Arduino_GFX::drawBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h, uint16_t color, uint16_t bg)
{
    int16_t byteWidth = (w + 7) / 8;  // Bitmap scanline pad = whole byte
    uint8_t byte      = 0;
...
*/
/**************************************************************************/
void Arduino_GFX::drawXBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h, uint16_t color)
{
    int16_t byteWidth = (w + 7) / 8;  // Bitmap scanline pad = whole byte
    uint8_t byte      = 0;
...
*/
/**************************************************************************/
void Arduino_GFX::drawGrayscaleBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h)
{
    uint8_t v;
    startWrite();
    for (int16_t j = 0; j < h; j++, y++)
...
*/
/**************************************************************************/
void Arduino_GFX::drawGrayscaleBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h)
{
    uint8_t v;
    startWrite();
    for (int16_t j = 0; j < h; j++, y++)
...
*/
/**************************************************************************/
void Arduino_GFX::drawGrayscaleBitmap(int16_t x, int16_t y, const uint8_t bitmap[], const uint8_t mask[], int16_t w, int16_t h)
{
...
*/
/**************************************************************************/
void Arduino_GFX::drawGrayscaleBitmap(int16_t x, int16_t y, uint8_t *bitmap, uint8_t *mask, int16_t w, int16_t h)
{
    int16_t bw   = (w + 7) / 8;  // Bitmask scanline pad = whole byte
    uint8_t byte = 0;
    uint8_t v;
...
*/
/**************************************************************************/
void Arduino_GFX::drawIndexedBitmap(int16_t x, int16_t y, uint8_t *bitmap, uint16_t *color_index, int16_t w, int16_t h, int16_t x_skip)
{
    int32_t offset = 0;
    startWrite();
    for (int16_t j = 0; j < h; j++, y++)
...
*/
/**************************************************************************/
void Arduino_GFX::drawIndexedBitmap(int16_t x, int16_t y, uint8_t *bitmap, uint16_t *color_index, uint8_t chroma_key, int16_t w, int16_t h, int16_t x_skip)
{
    int32_t offset = 0;
    uint8_t color_key;
    startWrite();
...
*/
/**************************************************************************/
void Arduino_GFX::draw3bitRGBBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h)
{
    ...
    for (int16_t j = 0; j < h; j++, y++)
    {
        for (int16_t i = 0; i < w; i++)
        {
            if (offset & 1)
            {
                d = (((c & 0b100) ? RGB565_RED : 0) | ((c & 0b010) ? RGB565_LIME : 0) | ((c & 0b001) ? RGB565_BLUE : 0));
            }
            else
            {
                c = bitmap[idx++];
                d = (((c & 0b100000) ? RGB565_RED : 0) | ((c & 0b010000) ? RGB565_LIME : 0) | ((c & 0b001000) ? RGB565_BLUE : 0));
            }
            writePixel(x + i, y, d);
            offset++;
        }
...
*/
/**************************************************************************/
void Arduino_GFX::draw16bitRGBBitmapWithTranColor(int16_t x, int16_t y, uint16_t *bitmap, uint16_t transparent_color, int16_t w, int16_t h)
{
    int32_t  offset = 0;
    uint16_t color;
    startWrite();
...
*/
/**************************************************************************/
void Arduino_GFX::draw16bitBeRGBBitmapR1(int16_t x, int16_t y, uint16_t *bitmap, int16_t w, int16_t h)
{
    int32_t  offset = 0;
    uint16_t p;
    startWrite();
...
*/
/**************************************************************************/
void Arduino_GFX::draw16bitRGBBitmapWithMask(int16_t x, int16_t y, const uint16_t bitmap[], const uint8_t mask[], int16_t w, int16_t h)
{
    int32_t offset = 0;
    int16_t bw     = (w + 7) / 8;  // Bitmask scanline pad = whole byte
    uint8_t byte   = 0;
...
*/
/**************************************************************************/
void Arduino_GFX::draw16bitRGBBitmapWithMask(int16_t x, int16_t y, uint16_t *bitmap, uint8_t *mask, int16_t w, int16_t h)
{
...
*/
/**************************************************************************/
void Arduino_GFX::draw24bitRGBBitmap(int16_t x, int16_t y, const uint8_t bitmap[], int16_t w, int16_t h)
{
    int32_t offset = 0;
    startWrite();
    for (int16_t j = 0; j < h; j++, y++)
...
*/
/**************************************************************************/
void Arduino_GFX::draw24bitRGBBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h)
{
    int32_t offset = 0;
    startWrite();
    for (int16_t j = 0; j < h; j++, y++)
...
*/
/**************************************************************************/
void Arduino_GFX::draw24bitRGBBitmap(int16_t x, int16_t y, const uint8_t bitmap[], const uint8_t mask[], int16_t w, int16_t h)
{
    int32_t offset = 0;
    int16_t bw     = (w + 7) / 8;  // Bitmask scanline pad = whole byte
    uint8_t byte   = 0;
...
*/
/**************************************************************************/
void Arduino_GFX::draw24bitRGBBitmap(int16_t x, int16_t y, uint8_t *bitmap, uint8_t *mask, int16_t w, int16_t h)
{
    int32_t offset = 0;
    int16_t bw     = (w + 7) / 8;  // Bitmask scanline pad = whole byte
    uint8_t byte   = 0;
...
void Arduino_GFX::drawChar(int16_t x, int16_t y, unsigned char c, uint16_t color, uint16_t bg)
{
    ...
    if (gfxFont)  // custom font
    {
        ...
        uint8_t  w = pgm_read_byte(&glyph->width), h = pgm_read_byte(&glyph->height), xAdvance = pgm_read_byte(&glyph->xAdvance), yAdvance = pgm_read_byte(&gfxFont->yAdvance),
    ...
    #else
        int8_t xo = pgm_read_sbyte(&glyph->xOffset), yo = pgm_read_sbyte(&glyph->yOffset);
    #endif
...
size_t Arduino_GFX::write(uint8_t c)
{
    ...
    if (gfxFont)  // custom font
    {
        ...
        {
            uint16_t first = pgm_read_word(&gfxFont->first), last = pgm_read_word(&gfxFont->last);
            if ((c >= first) && (c <= last))  // Char present in this font?
            {
                GFXglyph *glyph = pgm_read_glyph_ptr(gfxFont, c - first);
                uint8_t   gw = pgm_read_byte(&glyph->width), xa = pgm_read_byte(&glyph->xAdvance);
...
*/
/**************************************************************************/
void Arduino_GFX::setTextSize(uint8_t s) { setTextSize(s, s, 0); }
...
*/
/**************************************************************************/
void Arduino_GFX::setTextSize(uint8_t s_x, uint8_t s_y) { setTextSize(s_x, s_y, 0); }
...
void Arduino_GFX::setUTF8Print(bool isEnable) { _enableUTF8Print = isEnable; }
#endif  // defined(U8G2_FONT_SUPPORT)
...
*/
/**************************************************************************/
void Arduino_GFX::charBounds(char c, int16_t *x, int16_t *y, int16_t *minx, int16_t *miny, int16_t *maxx, int16_t *maxy)
{
#if !defined(ATTINY_CORE)
    if (gfxFont)  // custom font
    {
        ...
        else if (c != '\r')  // Not a carriage return; is normal char
        {
            ...
            if ((c >= first) && (c <= last))  // Char present in this font?
            {
                ...
                int16_t x1 = *x + ((int16_t)xo * textsize_x), y1 = *y + ((int16_t)yo * textsize_y), x2 = x1 + ((int16_t)gw * textsize_x) - 1, y2 = y1 + ((int16_t)gh * textsize_y) - 1;
                if (x1 < *minx)
                {
                    *minx = x1;
                }
        ...
        if (_utf8_state == 0)
        {
            ...
            {  // Ignore carriage returns
                ...
                if (glyph_data)
                {
                    ...
                    int16_t x1 = *x + ((int16_t)_u8g2_char_x * textsize_x), y1 = *y - (((int16_t)_u8g2_char_height + _u8g2_char_y) * textsize_y), x2 = x1 + ((int16_t)_u8g2_char_width * textsize_x) - 1,
...
*/
/**************************************************************************/
void Arduino_GFX::getTextBounds(const String &str, int16_t x, int16_t y, int16_t *x1, int16_t *y1, uint16_t *w, uint16_t *h)
{
    if (str.length() != 0)
    {
...
*/
/**************************************************************************/
void Arduino_GFX::getTextBounds(const __FlashStringHelper *str, int16_t x, int16_t y, int16_t *x1, int16_t *y1, uint16_t *w, uint16_t *h)
{
