...
void Arduino_Canvas_Indexed::writeFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color)
{
    uint8_t idx;
    if (_isDirectUseColorIndex)
    {
    ...
    switch (_rotation)
    {
        case 1: writeFastHLineCore(_height - y - h, x, h, idx); break;
        case 2: writeFastVLineCore(_max_x - x, _height - y - h, h, idx); break;
        case 3: writeFastHLineCore(y, _max_x - x, h, idx); break;
        default:  // case 0:
            writeFastVLineCore(x, y, h, idx);
    }
}

void Arduino_Canvas_Indexed::writeFastVLineCore(int16_t x, int16_t y, int16_t h, uint8_t idx)
{
...
void Arduino_Canvas_Indexed::writeFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color)
{
    uint8_t idx;
    if (_isDirectUseColorIndex)
    {
    ...
    switch (_rotation)
    {
        case 1: writeFastVLineCore(_max_y - y, x, w, idx); break;
        case 2: writeFastHLineCore(_width - x - w, _max_y - y, w, idx); break;
        case 3: writeFastVLineCore(y, _width - x - w, w, idx); break;
        default:  // case 0:
            writeFastHLineCore(x, y, w, idx);
    }
}

void Arduino_Canvas_Indexed::writeFastHLineCore(int16_t x, int16_t y, int16_t w, uint8_t idx)
{
...
void Arduino_Canvas_Indexed::writeFillRectPreclipped(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    uint8_t idx;
    if (_isDirectUseColorIndex)
    {
...
void Arduino_Canvas_Indexed::drawIndexedBitmap(int16_t x, int16_t y, uint8_t *bitmap, uint16_t *color_index, int16_t w, int16_t h, int16_t x_skip)
{
    if (_rotation > 0)
    {
        if (!_isDirectUseColorIndex)
...
void Arduino_Canvas_Indexed::drawIndexedBitmap(int16_t x, int16_t y, uint8_t *bitmap, uint16_t *color_index, uint8_t chroma_key, int16_t w, int16_t h, int16_t x_skip)
{
    if (_rotation > 0)
    {
        if (!_isDirectUseColorIndex)
...
uint8_t *Arduino_Canvas_Indexed::getFramebuffer() { return _framebuffer; }

uint16_t *Arduino_Canvas_Indexed::getColorIndex() { return _color_index; }

void Arduino_Canvas_Indexed::setDirectUseColorIndex(bool isEnable) { _isDirectUseColorIndex = isEnable; }
...
GFX_INLINE uint16_t Arduino_Canvas_Indexed::get_index_color(uint8_t idx) { return _color_index[idx]; }
