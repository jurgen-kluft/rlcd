...
Arduino_DSI_Display::Arduino_DSI_Display(int16_t w, int16_t h, Arduino_ESP32DSIPanel *dsipanel, uint8_t r, bool auto_flush, int8_t rst, const lcd_init_cmd_t *init_operations, size_t init_operations_len, uint8_t col_offset1, uint8_t row_offset1,
...
void Arduino_DSI_Display::draw16bitRGBBitmap(int16_t x, int16_t y, uint16_t *bitmap, int16_t w, int16_t h)
{
    if (_isRoundMode)
    {
...
void Arduino_DSI_Display::draw16bitBeRGBBitmap(int16_t x, int16_t y, uint16_t *bitmap, int16_t w, int16_t h)
{
    if (((x + w - 1) < 0) ||  // Outside left
        ((y + h - 1) < 0) ||  // Outside top
        (x > _max_x) ||       // Outside right
...
uint16_t *Arduino_DSI_Display::getFramebuffer() { return _framebuffer; }
