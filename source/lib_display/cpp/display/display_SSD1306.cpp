...
bool Arduino_SSD1306::begin(int32_t speed)
{
    ...
    static const uint8_t init_sequence[] = {BEGIN_WRITE,
                                            WRITE_COMMAND_BYTES,
                                            25,
                                            SSD1306_DISPLAYOFF,  // 0xAE
                                            SSD1306_SETCONTRAST,
                                            _contrast,                  // 0x81, contrast
                                            SSD1306_NORMALDISPLAY,      // 0xA6
                                            SSD1306_DEACTIVATE_SCROLL,  // 0x2E
                                            SSD1306_MEMORYMODE,
                                            0x00,                 // 0x20, 00 Horizontal addressing mode
                                            SSD1306_SEGREMAPINV,  // 0xA1
                                            SSD1306_SETMULTIPLEX,
                                            (uint8_t)(HEIGHT - 1),  // 0xA8, nn
                                            SSD1306_COMSCANDEC,     // 0xC8
                                            SSD1306_SETDISPLAYOFFSET,
                                            0x00,  // 0xD3, 00   no offset
                                            SSD1306_SETCOMPINS,
                                            comPins,  // 0xDA, nn
...
void Arduino_SSD1306::drawBitmap(int16_t x, int16_t y, uint8_t *bitmap, int16_t w, int16_t h, uint16_t /* color */, uint16_t /* bg */)
{
    ...
    // start page sequence
    _bus->beginWrite();
    uint8_t page_sequence[] = {SSD1306_PAGEADDR, 0, 0xFF, SSD1306_COLUMNADDR, _colStart, _colEnd};
...
void Arduino_SSD1306::displayOn(void) { _bus->sendCommand(SSD1306_DISPLAYON); }

void Arduino_SSD1306::displayOff(void) { _bus->sendCommand(SSD1306_DISPLAYOFF); }
