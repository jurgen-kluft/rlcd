...
void Arduino_ESP8266SPI::endWrite() { CS_HIGH(); }
...
void Arduino_ESP8266SPI::write(uint8_t d) { WRITE(d); }

void Arduino_ESP8266SPI::write16(uint16_t d) { WRITE16(d); }
...
void Arduino_ESP8266SPI::writeBytes(uint8_t *data, uint32_t len) { SPI.writeBytes(data, len); }

void Arduino_ESP8266SPI::writePattern(uint8_t *data, uint8_t len, uint32_t repeat) { SPI.writePattern(data, len, repeat); }
...
GFX_INLINE void Arduino_ESP8266SPI::DC_HIGH(void) { *_dcPort |= _dcPinMaskSet; }

GFX_INLINE void Arduino_ESP8266SPI::DC_LOW(void) { *_dcPort &= _dcPinMaskClr; }
