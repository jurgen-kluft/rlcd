...
void Arduino_RPiPicoSPI::endWrite() { CS_HIGH(); }
...
void Arduino_RPiPicoSPI::write(uint8_t d) { WRITE(d); }

void Arduino_RPiPicoSPI::write16(uint16_t d) { WRITE16(d); }
...
void Arduino_RPiPicoSPI::writeBytes(uint8_t *data, uint32_t len) { WRITEBUF(data, len); }

GFX_INLINE void Arduino_RPiPicoSPI::WRITE(uint8_t d) { spi_write_blocking(_spi, (const uint8_t *)&d, 1); }
...
GFX_INLINE void Arduino_RPiPicoSPI::WRITEBUF(uint8_t *buf, size_t count) { spi_write_blocking(_spi, (const uint8_t *)buf, count); }

/******** low level bit twiddling **********/

GFX_INLINE void Arduino_RPiPicoSPI::DC_HIGH(void) { *_dcPortSet = _dcPinMask; }

GFX_INLINE void Arduino_RPiPicoSPI::DC_LOW(void) { *_dcPortClr = _dcPinMask; }
