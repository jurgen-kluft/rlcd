...
void Arduino_mbedSPI::write(uint8_t d) { WRITE(d); }

void Arduino_mbedSPI::write16(uint16_t d) { WRITE16(d); }
...
void Arduino_mbedSPI::writeBytes(uint8_t *data, uint32_t len) { WRITEBUF(data, len); }

GFX_INLINE void Arduino_mbedSPI::WRITE(uint8_t d) { _dev->write((const char *)&d, 1, NULL, 0); }
...
GFX_INLINE void Arduino_mbedSPI::WRITEBUF(uint8_t *buf, size_t count) { _dev->write((const char *)buf, count, NULL, 0); }

/******** low level bit twiddling **********/

GFX_INLINE void Arduino_mbedSPI::DC_HIGH(void) { *_dcPortSet = _dcPinMask; }

GFX_INLINE void Arduino_mbedSPI::DC_LOW(void) { *_dcPortClr = _dcPinMask; }
