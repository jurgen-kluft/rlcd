...
void Arduino_DUEPAR16::endWrite() { CS_HIGH(); }
...
void Arduino_DUEPAR16::write(uint8_t d) { WRITE16(d); }

void Arduino_DUEPAR16::write16(uint16_t d) { WRITE16(d); }

void Arduino_DUEPAR16::writeRepeat(uint16_t p, uint32_t len) { writeData16(p, len); }
...
GFX_INLINE void Arduino_DUEPAR16::WRITE16(uint16_t d) { writeData16(d, 1); }
...
void Arduino_DUEPAR16::writeData16(uint16_t d, uint32_t num)
{
    ...
    if (d & 0x8000)
        REG_PIOD_SODR = 0x1 << 6;  // D.6
    if (d & 0x4000)
        REG_PIOD_SODR = 0x1 << 3;  // D.3
    if (d & 0x2000)
        REG_PIOD_SODR = 0x1 << 2;  // D.2
    if (d & 0x1000)
        REG_PIOD_SODR = 0x1 << 1;  // D.1
    if (d & 0x0800)
        REG_PIOD_SODR = 0x1 << 0;  // D.0
    if (d & 0x0400)
        REG_PIOA_SODR = 0x1 << 15;  // A.15
    if (d & 0x0200)
        REG_PIOA_SODR = 0x1 << 14;  // A.14
    if (d & 0x0100)
        REG_PIOB_SODR = 0x1 << 26;  // B.26

    // so it is quite quick.                    Port.bit
    if (d & 0x0080)
        REG_PIOD_SODR = 0x1 << 9;  // D.9
    if (d & 0x0040)
        REG_PIOA_SODR = 0x1 << 7;  // A.7
    if (d & 0x0020)
        REG_PIOD_SODR = 0x1 << 10;  // D.10
    if (d & 0x0010)
        REG_PIOC_SODR = 0x1 << 1;  // C.1
    if (d & 0x0008)
        REG_PIOC_SODR = 0x1 << 2;  // C.2
    if (d & 0x0004)
        REG_PIOC_SODR = 0x1 << 3;  // C.3
    if (d & 0x0002)
        REG_PIOC_SODR = 0x1 << 4;  // C.4
    if (d & 0x0001)
        REG_PIOC_SODR = 0x1 << 5;  // C.5
    while (num > 0)
    {
        // WR_STB;
        REG_PIOC_CODR = 0x1 << 7;
...
GFX_INLINE void Arduino_DUEPAR16::DC_HIGH(void) { digitalWrite(_dc, HIGH); }

GFX_INLINE void Arduino_DUEPAR16::DC_LOW(void) { digitalWrite(_dc, LOW); }

GFX_INLINE void Arduino_DUEPAR16::CS_HIGH(void) { digitalWrite(_cs, HIGH); }

GFX_INLINE void Arduino_DUEPAR16::CS_LOW(void) { digitalWrite(_cs, LOW); }
