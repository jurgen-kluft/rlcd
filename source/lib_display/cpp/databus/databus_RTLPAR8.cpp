...
Arduino_RTLPAR8::Arduino_RTLPAR8(int8_t dc, int8_t cs, int8_t wr, int8_t rd, int8_t d0, int8_t d1, int8_t d2, int8_t d3, int8_t d4, int8_t d5, int8_t d6, int8_t d7)
    : _dc(dc)
    , _cs(cs)
    , _wr(wr)
    , _rd(rd)
...
void Arduino_RTLPAR8::endWrite() { CS_HIGH(); }
...
void Arduino_RTLPAR8::write(uint8_t d) { WRITE(d); }
...
GFX_INLINE void Arduino_RTLPAR8::DC_HIGH(void) { *_dcPort |= _dcPinMaskSet; }

GFX_INLINE void Arduino_RTLPAR8::DC_LOW(void) { *_dcPort &= _dcPinMaskClr; }
