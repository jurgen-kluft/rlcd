...
void Arduino_NRFXSPI::endWrite() { CS_HIGH(); }
...
void Arduino_NRFXSPI::write(uint8_t d) { WRITE(d); }

void Arduino_NRFXSPI::write16(uint16_t d) { WRITE16(d); }
...
void Arduino_NRFXSPI::writeBytes(uint8_t *data, uint32_t len) { WRITEBUF(data, len); }
...
GFX_INLINE void Arduino_NRFXSPI::DC_HIGH(void) { *_dcPortSet = _dcPinMask; }

GFX_INLINE void Arduino_NRFXSPI::DC_LOW(void) { *_dcPortClr = _dcPinMask; }
