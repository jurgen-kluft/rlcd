...
esp_err_t esp_lcd_new_panel_sh8601(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    {
        case LCD_RGB_ELEMENT_ORDER_RGB: sh8601->madctl_val = 0; break;
        case LCD_RGB_ELEMENT_ORDER_BGR: sh8601->madctl_val |= LCD_CMD_BGR_BIT; break;
        default: ESP_GOTO_ON_FALSE(false, ESP_ERR_NOT_SUPPORTED, err, TAG, "unsupported color element order"); break;
    }
...
static esp_err_t panel_sh8601_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    x_start += sh8601->x_gap + 0x16;
    x_end += sh8601->x_gap + 0x16;
    y_start += sh8601->y_gap;
    y_end += sh8601->y_gap;

    // define an area of frame memory where MCU can access
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_CASET,
                                 (uint8_t[]){
                                   (x_start >> 8) & 0xFF,
                                   x_start & 0xFF,
                                   ((x_end - 1) >> 8) & 0xFF,
    ...
    ESP_RETURN_ON_ERROR(tx_param(sh8601, io, LCD_CMD_RASET,
                                 (uint8_t[]){
                                   (y_start >> 8) & 0xFF,
                                   y_start & 0xFF,
                                   ((y_end - 1) >> 8) & 0xFF,
